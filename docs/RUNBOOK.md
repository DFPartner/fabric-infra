...
### Grafana
The makefile includes a helper:
```bash
make monitoring
```
Or manually:
...
*Note: This setup does not yet use a GitOps secret manager (like Sealed Secrets or Vault). Secrets in Git are currently limited to non-sensitive configuration or require manual creation.*
