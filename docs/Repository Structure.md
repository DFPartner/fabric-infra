...
│   ├── bp-dsk/         # Configurations for the 'bp-dsk' cluster
│   │   └── applications/ # Argo CD Application manifests enabled for this cluster
│   └── df-test/        # Configurations for the 'df-test' cluster
│       └── applications/
...
├── root-app-bp-dsk.yaml   # The Entry Point for bp-dsk cluster
└── root-app-df-test.yaml  # The Entry Point for df-test cluster
```
...
-   `00-install-base.sh`: A shell script (runbook) that:
...
-   `02-projects-*.yaml`: Defines Argo CD Projects (e.g., `platform-ops`) specific to each cluster to group applications and set access policies.

### 2. The Root Application (`root-app-*.yaml`)
This is the "App of Apps". There is one per cluster (e.g., `root-app-bp-dsk.yaml`). It is an Argo CD Application resource that monitors the corresponding `clusters/<cluster-name>/applications/` directory.
-   **Recursive Sync:** It is configured with `recurse: true`, meaning it will find all `.yaml` files inside the target applications directory.
-   **Self-Management:** It manages the lifecycle of the child applications.

### 3. Cluster Applications (`clusters/<cluster>/applications/`)
This directory contains the definitions for the actual applications enabled for a specific cluster. Each file here is an `Application` CRD.
-   **Example:** `clusters/bp-dsk/applications/data-weed.yaml`
...
2.  **Detection:** Argo CD (via the `mon-alloy` Application defined in `clusters/<cluster>/applications/mon-alloy.yaml`) detects that the source path `platform/monitoring/alloy` has changed.
